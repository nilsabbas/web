 ...
 * @return array
 */
function nanda_body_classes( $classes ) {
...
add_filter( 'body_class', 'nanda_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function nanda_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'nanda_pingback_header' );
 ...
 * @since 1.0.0
 */
if( ! function_exists( 'nanda_font_awesome_social_icon_array' ) ) :
    function nanda_font_awesome_social_icon_array(){
        return array(
...
if(!function_exists('nanda_advertisement_items')):
    function nanda_advertisement_items(){  
        $get_frontpage_product_advertisement_items = get_theme_mod( 'nanda_frontpage_product_advertisement_items', '' );
    $get_decode_frontpage_product_advertisement_items = json_decode( $get_frontpage_product_advertisement_items );
    if( ! empty( $get_decode_frontpage_product_advertisement_items ) ) {
        foreach ( $get_decode_frontpage_product_advertisement_items as $single_item ) {
            $product_title  = $single_item->mt_item_text;
            $product_info  = $single_item->mt_item_text1;
            $product_price  = $single_item->mt_item_text2;
            $product_image = $single_item->mt_item_upload;
            ?>
            <div class="col-md-6">
                <div class="l-holder">
                  <a href="#"><img src="<?php echo esc_url( $product_image ); ?>" alt=""></a>
                  <div class="caption">
                    <span class="t-title"><?php echo esc_html($product_title);?></span>
                    <h4 class="b-title"><?php echo esc_html($product_info);?></h4>
                    <span class="price-tag">$ <span class="b-tag"><?php echo esc_html($product_price);?></span></span>
                </div>
            </div>
        </div>
       <?php
 ...
 if ( ! function_exists( 'nanda_is_woocommerce_activated' ) ) {
    function nanda_is_woocommerce_activated() {
...
if ( ! function_exists( 'nanda_inner_breadcrumb' ) ) :
     ...
     * @since 1.0.0
     */
    function nanda_inner_breadcrumb() {
