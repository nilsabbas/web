...
if(!empty($product_category)):
    ...
    $args = array(
        ...
        'meta_query' => array(
            array(
                'key' => 'about_section_product_category', // Custom field key "groupname_fieldname"
                'value' => $postID, // Custom field value
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) {
            $query->the_post();
            $relPostID =  get_the_ID() ;
            array_push( $RpIds, $relPostID );
        }
        wp_reset_postdata();
    }
...
<div class="na-partileset">
    <div class="content-wrapper" id="range">
        <div>
            <div class="na-heading na-heading--inv">
                <h2 class="na-heading__title_prod"><?= $relative_products_title; ?></h2>
            </div>
        </div>
        <div class="na-partileset__sub">
            ...
            ?>
            <div class="na-partile" style="<?php echo $show_block; ?>">
                <a href="<?= esc_url($relPostUrl); ?>/?product=true" class="na-partile__black">
            ...
            <div class="not-found" style="<?php echo $notfound; ?>"><h5>No products available.</h5></div>
            <?php }
            // endif;
            ?>
...
<!-- JavaScript with Offset Scroll-->
<script>
//Trigger scroll on click once
//document.addEventListener('DOMContentLoaded', function () {
//    const productTab = document.getElementById('pills-product-tab');
//    const ranSection = document.getElementById('range');
//
//    productTab.addEventListener('shown.bs.tab', function () {
//        if (ranSection) {
//            const yOffset = -100;  Negative offset to leave space from top
//            const y = ranSection.getBoundingClientRect().top + window.pageYOffset + yOffset;
//
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Also keep Bootstrap tab behavior
    productTab.addEventListener('shown.bs.tab', function () {
        scrollToSection();
    });
});
